...
// Fixed-capacity single-producer/single-consumer interleaved Float32 PCM
// queue. configure() and clear() require the consumer to be stopped. push()
// and pop() may then run concurrently without locks or allocation.
class InterleavedFloatPcmQueue final {
public:
    struct PopResult {
        int frames = 0;
