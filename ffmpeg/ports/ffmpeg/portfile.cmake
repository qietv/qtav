...
file(COPY
    "${CMAKE_CURRENT_LIST_DIR}/libsmb2-overlay/libavformat/libsmb2.c"
    "${CMAKE_CURRENT_LIST_DIR}/libsmb2-overlay/libavformat/libsmb2url.c"
    "${CMAKE_CURRENT_LIST_DIR}/libsmb2-overlay/libavformat/libsmb2url.h"
    DESTINATION "${SOURCE_PATH}/libavformat"
)
file(COPY
    "${CMAKE_CURRENT_LIST_DIR}/libsmb2-overlay/libavformat/tests/libsmb2url.c"
    DESTINATION "${SOURCE_PATH}/libavformat/tests"
)

# The upstream vcpkg dependency patch adds libshaderc between libshine and
# libsmbclient, so keep the avbuild2 libsmb2 patch small and inject its
# configure probe after the complete vcpkg patch series has been applied.
file(READ "${SOURCE_PATH}/configure" FFMPEG_CONFIGURE)
set(LIBSMB2_CONFIGURE_ANCHOR
    "enabled libshine          && require_pkg_config libshine shine shine/layer3.h shine_encode_buffer\n"
)
set(LIBSMB2_CONFIGURE_PROBE
    "enabled libsmb2           && require_pkg_config libsmb2 \"libsmb2 >= 6.1.0\" \"stdint.h stddef.h time.h smb2/smb2.h smb2/libsmb2.h\" smb2_init_context\n"
)
string(FIND "${FFMPEG_CONFIGURE}" "${LIBSMB2_CONFIGURE_PROBE}" LIBSMB2_PROBE_POSITION)
if(LIBSMB2_PROBE_POSITION EQUAL -1)
    string(FIND "${FFMPEG_CONFIGURE}" "${LIBSMB2_CONFIGURE_ANCHOR}" LIBSMB2_ANCHOR_POSITION)
    if(LIBSMB2_ANCHOR_POSITION EQUAL -1)
        message(FATAL_ERROR "Could not locate the FFmpeg libshine configure probe")
    endif()
    string(REPLACE
        "${LIBSMB2_CONFIGURE_ANCHOR}"
        "${LIBSMB2_CONFIGURE_ANCHOR}${LIBSMB2_CONFIGURE_PROBE}"
        FFMPEG_CONFIGURE
        "${FFMPEG_CONFIGURE}"
    )
endif()

set(LIBSMBCLIENT_CONFIGURE_ANCHOR
    "                               require libsmbclient libsmbclient.h smbc_init -lsmbclient; }\n"
)
set(LIBSMB2_CONFIGURE_CONFLICT
    "enabled libsmb2 && enabled libsmbclient && die \"ERROR: libsmb2 and libsmbclient cannot both provide the smb protocol\"\n"
)
string(FIND "${FFMPEG_CONFIGURE}" "${LIBSMB2_CONFIGURE_CONFLICT}" LIBSMB2_CONFLICT_POSITION)
if(LIBSMB2_CONFLICT_POSITION EQUAL -1)
    string(FIND "${FFMPEG_CONFIGURE}" "${LIBSMBCLIENT_CONFIGURE_ANCHOR}" LIBSMBCLIENT_ANCHOR_POSITION)
    if(LIBSMBCLIENT_ANCHOR_POSITION EQUAL -1)
        message(FATAL_ERROR "Could not locate the FFmpeg libsmbclient configure probe")
    endif()
    string(REPLACE
        "${LIBSMBCLIENT_CONFIGURE_ANCHOR}"
        "${LIBSMBCLIENT_CONFIGURE_ANCHOR}${LIBSMB2_CONFIGURE_CONFLICT}"
        FFMPEG_CONFIGURE
        "${FFMPEG_CONFIGURE}"
    )
endif()
...
if("libsmb2" IN_LIST FEATURES)
    set(OPTIONS "${OPTIONS} --enable-libsmb2")
else()
    set(OPTIONS "${OPTIONS} --disable-libsmb2")
endif()

if("libplacebo" IN_LIST FEATURES)
